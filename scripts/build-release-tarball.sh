...
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    CLRP_HASH="$(git rev-parse --short=7 HEAD)"
fi
if git -C "$EB_SUB" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    EB_HASH="$(git -C "$EB_SUB" rev-parse --short=7 HEAD)"
fi
...
EGS_CLRP_HASH = -DEGS_CLRP_HASH="\"${CLRP_HASH}\""
EGS_BRACHY_HASH = -DEGS_BRACHY_HASH="\"${EB_HASH}\""
EOF

tar -czf "$ARCHIVE" -C "$OUT_DIR" "$DIR"
echo "Wrote $ARCHIVE"
echo "  $(du -h "$ARCHIVE" | cut -f1)  $(tar -tzf "$ARCHIVE" | wc -l | tr -d ' ') files"
echo "  release.mk: EGS_RELEASE=${VERSION} CLRP=${CLRP_HASH:-?} egs_brachy=${EB_HASH:-?}"
